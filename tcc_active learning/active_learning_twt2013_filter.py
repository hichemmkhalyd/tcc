...
def benchmark():
        ...
        if ACTIVE == 2:
           df_filtered = unlabeled_filter()
           df_filtered.pol = clf.predict(vectorizer.transform(df_filtered.text))
           
           df_unified = df_train.append(df_filtered)
           X_unified  = vectorizer.transform(df_unified.text)
           clf.fit(X_unified, df_unified.pol)
...
def unlabeled_filter():
    df_text = df_unlabeled
    confidences = clf.decision_function(X_unlabeled)
    avarege = [np.mean(confidences[:,0]),
               np.mean(confidences[:,1]),np.mean(confidences[:,2])]
    
    for i in range(0,len(avarege)):
        if avarege[i] < 0:
            avarege[i] = 0
    
    for i in range(0,len(confidences)):
        if sum(confidences[i] > avarege) != 1:
            df_text = df_text.drop(i)
...
clf = LinearSVC(loss='l2', penalty='l2',
                dual=True, tol=1e-3,class_weight='balanced')
...
ENCODING = 'utf-8'
vectorizer = TfidfVectorizer(encoding= ENCODING, use_idf=True, norm='l2', binary=True, sublinear_tf=True,min_df=0.0001, max_df=1.0, ngram_range=(1, 2), analyzer='word', stop_words=None)
result_x = []
result_y = []
###############################################################################
